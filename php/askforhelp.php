...
elseif(isset($_POST["submit"])){
  ...
  // if good to go, uploads 
      ...
      if (move_uploaded_file($_FILES["id_proof"]["tmp_name"], $target_file)) {
        ...
        $stmt = $conn->prepare("INSERT INTO help (name, email, mobile, address, DOB, id_name, id_type, id_proof) 
		values(?,?,?,?,?,?,?,?)");
  	    $stmt->bind_param("ssisssss", $name, $email, $mobile, $address, $DOB, $filename, $type, $data);
        $stmt->execute();
        echo "<script>alert('WE WILL REACH YOU SOON. STAY SAFE, STAY INDOORS');</script>";
        $stmt->close();
        $conn->close();
  }   else {
